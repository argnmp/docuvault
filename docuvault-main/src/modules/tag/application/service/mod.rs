...
pub struct TagSetService {
    tag_set_port: Box<dyn TagSetRepositoryPort + Send + Sync> 
}
impl TagSetService {
    pub fn new(tag_set_port: Box<dyn TagSetRepositoryPort + Send + Sync>) -> Self {
        Self {
            tag_set_port,
        }
    }
...
impl TagSetUseCase for TagSetService {
    async fn get(&self) -> Result<TagSet, GlobalError> {
        let tag_set = self.tag_set_port.load().await?;
        Ok(tag_set)
    }
    async fn add(&self, tag: String) -> Result<(), GlobalError> {
        let mut tag_set = self.tag_set_port.load().await?; 
        tag_set.add_tag(Tag::new(tag));
        self.tag_set_port.save(&tag_set).await?;
        Ok(())
    }
